...
\begin{spacing}{1} %1.9
	\begin{abstract} %EMW8Apr: Some edits to the abstract below, see what you think. I also think you should provide a bit more results. You write 'We find only isolated effects of population origin.' -- do you mean only a few species or ...? I would set up more your results and whether they are consistent across species. You should also see if you can work in your finding that it's strat x temp that matters? That's an interesting finding and might be worth fitting in if you can. You may need to re-work the abstract to make it fit and still flow. 
...
Discovering the determinants of invasion success requires more information on how these two techniques drive essential invasion traits, including germination rate, germination timing, and growth rate. 
Here, we tested for evidence of evolution in these traits by using growth chambers to provide common environments for seven herbaceous plant species sampled from multiple populations in their native (European) and invasive (North American) ranges. Chambers provided two levels of stratification (to simulate different winter lengths) and four levels post-stratification (to simulate different spring conditions). Bayesian multilevel models allowed us to examine responses for each species, as well as across the suite of all seven species.
We find only isolated effects of population origin. This suggests that germination and growth traits of exotic species need not evolve to become successful invaders. Instead, our results suggest that broad environmental tolerance underlies invasion success for this suite of common invaders. 
	\end{abstract}
\end{spacing}		
	\section{Introduction} %EMW8Apr: Intro is much better, but still needs to be much shorter -- suggestions below. If you feel strongly about stuff you want to leave in, I suggest finding other possible edits. 
	Exotic plant invasions  can transform biodiversity \parencite{Bellard2016,Clavero2005,Walker1997}, and ecosystem function, services, and resilience \parencite{Daehler1999,Daehler1994,Ehrenfeld2003,Wilcove1998,Pejchar2009,Pimentel2005,Pysek2010,OTA1993,Mack2000,Levine2003}.  %EMW8Apr: I am not sure what I meant either! Delete all these comments ... 
...
To understand the drivers of these invasions, biologists have identified two key biological mechanisms that plants may use to invade.  
...
%EMW8Apr: I would make the above paragraph shorter, maybe potentially merge with first short paragraph? (Also, I cut the 'Here, we test the ...' as readers generally expect that closer to the end of the intro.)	
...
%%EMW8Apr: Nice!
	...
	%EMW8Apr: That's fine -- I still think it should be shorter and the following paragraph also... 
...
%EMW8Apr: Below paragraph is nice!	
...
%EMW8Apr: Below paragraph could be shorter. 	
...
%EMW8Apr: The below introduction of the Concord data is very nice! Can you move these 1-2 sentences up to the intro perhaps? Not 100% sure it will work, but I suggest trying it.
...
%	\end{figure}
	...
	\subsection{Stratification} %EMW8Apr: should be parencite below?
	...
	\subsection{Statistical analysis} %EMW8Apr: Some edits below, see what you think.
...
%EMW8Apr: I would move this up to where you mention how you measured height; it is statistics, but it feels more lower-level and thus is a little confusing here. 
	Plant height was roughly linear with time (see Figure \ref{fig:lmgr}), so growth rate was defined as $\beta$ in the linear model: $height = \alpha + \beta*day $. This growth rate was calculated for each seed that germinated. 
%EMW8Apr: Check edits below.
For all models (growth rate, germination rate, and germination timing), stratification length, continental origin, and temperature were treated as binary fixed effects, with the full suite of 2- and 3-way interactions included. Europe, 18/8$^\circ$C, and 30 days were reference levels for origin, temperature, and stratification length, respectively; temperature was recoded as three dummy binary factors, allowing non-linear responses to treatments. Seed family was treated as a random effect, nested within sampling population, nested within species (with random slopes and intercepts). Growth rate was modeled with a normal error distribution: 
...
\end{align}
 ...
 %(95\% CrI: \Sexpr{round(invlogit(mod_rate$stan_summary[2,4]),2)}--\Sexpr{round(invlogit(mod_rate$stan_summary[2,10]),2)})
	...
	\paragraph{Growth rate} The mean growth rate was \Sexpr{round(mean(datax$y, na.rm=TRUE)*10,1)} mm/day. Overall, growth rate was the most sensitive to treatments, though it was still unaffected by stratification length or population origin (see Figures \ref{fig:coef}, \ref{fig:rawgrowth}; Table \ref{tab:mod_gr}). Growth rate decreased at warmer temperatures for all species, but especially DACGLO. This decreasing response to temperature contrasts with consistent effects of temperature for germination timing (see consistent effects of  Table \ref{tab:apc}).
%EMW8Apr: The above is tricky to explain, see my edits and perhaps we can do better. I think we want to avoid saying 'linear' if we can as readers will then expect a linear regression or such...Also -- the table looks like growth rate goes UP so we may need to stress what the APCs tell readers more in the caption... 
%EMW8Apr: Could you move the essence of this next sentence below to the APC table (and cut it here)? I think readers will want more help interpreting this table and thus I think this contrast could be best explained in the caption. 
...
% EMW8Apr: for the below ... is it also accurate to say: This decreased growth rate at high temperatures was not uniform across treatments:  seeds stratified for 60 days and originating in North America (the invasive range) grew XX faster per day [faster compared to what? The mean? Or the intercept? ... in which case I would say faster per day than seeds from Europe with only 30 days strat ... and then you might also want to calculate give the XX faster than European plants with 60 days strat....]
However, the growth rate decrease at the highest temperatures was moderated in seeds stratified for 60 days and originating in North America (the invasive range): these plants grew \Sexpr{round(mod_gr$stan_summary[15,7]*10,2)}mm faster per day (95\% CrI: \Sexpr{round(mod_gr$stan_summary[15,4]*10,2)}--\Sexpr{round(mod_gr$stan_summary[15,10]*10,2)}) (see `origin $\times$ strat $\times$ temp2' in Table \ref{tab:mod_gr}). 
...
\begin{figure}
	\fbox{\includegraphics[scale=.5,angle=-90]{germ_figs_onepage.pdf}} %EMW-Mar2020: Perhaps we should have a raw data figure in the supp? Similar to what you have in your thesis?
...
%EMW8Apr: Great points below! I tried to soften the language so we get the same message across but make it sound more inviting. I think the sentence starting with 'Furthermore' needs to be simpler and clearer ... I am not sure the best fix but perhaps something along the lines of: Our results suggest considering the invasive range's multivariate environment may be critical to predicting which traits  evolve after introduction. We found that traits are most likely to evolve in response to specific combinations of spring temperature and winter length.
	These results highlight the need to condition biological invasion mechanisms on specific invasion traits. We found that rapid evolution played no role in germination rate, but did play a limited role in growth rate for these species. This suggests that research and theory aimed at identifying which traits are likely to rapidly evolve to enable invasion may be provide more progress than testing for an overall mechanism of invasion that is consistent across traits. Furthermore, the types of environmental variables in which these traits evolve in response to can also be illuminating. Our results suggest that traits are most likely to evolve in response to specific combinations of spring temperature and winter length.  Adaptation to winter length is especially under-studied, and our results suggest that examining it is only useful in concert with other climate variables.  Not only can these trait evolution/environment relationships be useful for understanding invasions, they can also help delineate plant capacities to adapt to anthropogenic climate change. %EMW8Apr: Last couple sentences here are great!
	
	Our findings preliminarily suggest that these invasive species may be able to adapt to changing climates. Because cold-stratification is a simulation of temperate winter, the evidence that species can adapt their growth rate in accordance with winter length and spring temperature suggests that they may have the capacity to adapt to changing winter lengths and spring temperatures that anthropogenic climate change is bringing \parencite{IPCC2015}. These results also echo the  importance of varying both winter length and spring temperature in order to observe responses to climatic \parencite[e.g.,][]{Bernareggi2016}. %EMW8Apr: noun missing here I think....

%EMW8Apr: A few edits below, but I think you could do a better job of highlighting your limitations while also (1) making them seem less limiting (you had this already, I just think we could make it sound stronger) and (2) re-stating some important points. I started work on this, but I think it could be much improved!

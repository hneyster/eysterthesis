...
\begin{spacing}{1} %1.9
	\begin{abstract} %EMW8Apr: Some edits to the abstract below, see what you think. I also think you should provide a bit more results. You write 'We find only isolated effects of population origin.' -- do you mean only a few species or ...? I would set up more your results and whether they are consistent across species. You should also see if you can work in your finding that it's strat x temp that matters? That's an interesting finding and might be worth fitting in if you can. You may need to re-work the abstract to make it fit and still flow. 
		%HNE15Apr: Thanks for the edits! Added a bit more to the results. 
...
\end{spacing}		
	...
	%HNE15Apr: shortened significantly! 
...
\end{align}
 ...
 %(95\% CrI: \Sexpr{round(invlogit(mod_rate$stan_summary[2,4]),2)}--\Sexpr{round(invlogit(mod_rate$stan_summary[2,10]),2)})
	...
	\paragraph{Growth rate} The mean growth rate was \Sexpr{round(mean(datax$y, na.rm=TRUE)*10,1)} mm/day. Overall, growth rate was the most sensitive to treatments, though it was still unaffected by stratification length or population origin (see Figures \ref{fig:coef}, \ref{fig:rawgrowth}; Table \ref{tab:mod_gr}). Growth rate decreased at warmer temperatures for all species, but especially DACGLO. This decrease got progressively bigger for each higher temperature; this is in contrast to germination timing, where the decrease with temperature was more constant (see comparison in absolute change displayed in Table \ref{tab:apc}). 
...
%HNE15Apr: Great to see the way you modifed the first paragraph!  I tried to emulate in the second para. 

...
(ntot = nsite*ntemp*nstrat*rep) # 792 rows; 22k rows across species
...
siteorigin = 0
sitestrat = 0
temporigin = 3.5 # positive 3.5. So at the temp level, the effect of longer days is muted by 3.5 days.
tempstrat = 11 # both positive ~ 10. 
originstrat = 0.1 # from stan results
...
coeff <- c(1, sitediff, tempdiff, origindiff,  stratdiff, 
           sitetemp, siteorigin, sitestrat,
           temporigin, tempstrat,
           originstrat
)
...
for(i in 1:nsp){ # loop over species
  ...
  # Give variation for individual seeds within species in the intercept
  indint <- spint[i] + (1:nind-mean(1:nind)) #this variation seems a bit excessive... 
